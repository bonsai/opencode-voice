...
import { setTtsPlaying } from "./stt.js";
...
const TTS_VOICES = {
  jp_hi_fi_captain: {
    label: "Japanese hi-fi-captain (medium)",
...
const DEFAULT_TTS_VOICE = "jp_hi_fi_captain";

const PIPER_RATE = 22050;
const PIPER_BITS = 16;
const PIPER_CHANNELS = 1;

// ---- High-frequency TTS playback squelch tone ----
// While TTS plays we add a continuous low-level ~7kHz sine to the output. The
// STT side (lib/stt.js) detects that tone in the mic signal (16kHz sample rate,
// so 7kHz is just below Nyquist) and skips transcription, so the assistant's
// own voice is never transcribed as user input.
const TONE_FREQ = 7000;
const TONE_GAIN = 0.18; // fraction of full scale
const TONE_PHASE_STEP = (2 * Math.PI * TONE_FREQ) / PIPER_RATE;
let tonePhase = 0;

function mixTone(chunk, kv) {
  if (!kv?.get("tts.tone", true) || TONE_GAIN <= 0) return chunk;
  const out = Buffer.from(chunk);
  for (let i = 0; i + 1 < out.length; i += 2) {
    const sample = out.readInt16LE(i);
    const tone = Math.sin(tonePhase) * TONE_GAIN * 32768;
    tonePhase += TONE_PHASE_STEP;
    if (tonePhase >= Math.PI * 2) tonePhase -= Math.PI * 2;
...
export function registerTTS(api, kv, complete, prompts) {
    ...
    return new Promise((resolve) => {
      ...
      piperProc.stdout.on("data", (chunk) => {
        if (playProc?.stdin && !playProc.stdin.destroyed) {
          playProc.stdin.write(mixTone(chunk, kv));
        }
      });
  ...
  function getTtsMode() {
    ...
    return TTS_MODES.some((m) => m.value === mode) ? mode : "off";
  }
            ...
            options: [
              {
                ...
                onSelect() {
                  api.ui.dialog.clear();
                  speakWithSessionPrefix(sessionID, llmResult.text, "Ready for your input.");
                },
              },
              {
                title: "読み上げない",
  ...
  return [
    ...
    {
      title: "TTS: toggle mode",
      value: "tts.mode",
      description: "Toggle auto TTS: always / select / off",
      keybind: "<leader>v",
      slash: { name: "tts-mode" },
      onSelect() {
        const current = getTtsMode();
        const idx = TTS_MODES.findIndex((m) => m.value === current);
        const next = TTS_MODES[(idx + 1) % TTS_MODES.length];
        kv.set("tts.mode", next.value);
        if (next.value === "off") stopSpeech();
        const voice =
          TTS_VOICES[kv.get("tts.voice", DEFAULT_TTS_VOICE)] || TTS_VOICES[DEFAULT_TTS_VOICE];
        toast(`TTS: ${next.label} (${voice.label})`);
      },
    },
    {
