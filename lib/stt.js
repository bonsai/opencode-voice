...
// ---- High-frequency TTS squelch tone detection ----
// TTS (lib/tts.js) mixes a continuous ~7kHz sine into its output. Here we run a
// Goertzel algorithm over the recorded PCM (16kHz mono 16-bit) to detect that
// tone; if present, the recording is the assistant's own voice and must be
// skipped instead of transcribed.
const TONE_FREQ = 7000;
const TONE_SAMPLE_RATE = 16000;
const TONE_MIN_BIN_LEVEL = 1200; // Goertzel magnitude (avg per sample) to count as tone

function goertzelToneLevel(int16Buf, freq = TONE_FREQ, sampleRate = TONE_SAMPLE_RATE) {
  const n = Math.floor(int16Buf.length / 2);
  if (n < 8) return 0;
  ...
  let s2 = 0;
  for (let i = 0; i < n; i++) {
    s0 = int16Buf.readInt16LE(i * 2) + coeff * s1 - s2;
    s2 = s1;
    s1 = s0;
  }
...
function wavHasTtsTone() {
  try {
    if (!fs.existsSync(WAV_FILE)) return false;
    const wav = fs.readFileSync(WAV_FILE);
    if (wav.length <= 44) return false;
    const pcm = wav.subarray(44);
    return goertzelToneLevel(pcm) >= TONE_MIN_BIN_LEVEL;
  } catch {
    return false;
  }
}
...
// ---- Hands-free auto (VAD) mode ----
// Continuously samples the mic, starts recording when speech is detected,
// and auto-transcribes (and optionally submits) once silence is heard.
// No keybind required after enabling (/stt-auto).

const AUTO_THRESHOLD_DB = -42.0; // above this RMS the mic is "speaking"
const AUTO_START_BLOCKS = 3; // ~300ms of continuous speech to begin recording
const AUTO_END_BLOCKS = 8; // ~800ms of silence to stop and transcribe
const AUTO_MAX_MS = 30000; // safety cap for an uninterrupted utterance

let autoEnabled = false;
let autoProc = null;
let autoBuf = Buffer.alloc(0);
let autoSpeechStreak = 0;
let autoSilenceStreak = 0;
let autoHadSpeech = false;
let autoRecordingSince = null;
...
async function doTranscribePipeline(kv, complete, client, toast, systemPrompt, submit = false) {
  ...
  try {
    ...
    if (kv.get("stt.toneMute", true) && wavHasTtsTone()) {
      toast("TTS再生中 - 文字起こしをスキップしました", "warning");
      return;
    }
...
function stopAuto() {
  ...
  if (autoProc) {
    try {
      autoProc.kill("SIGTERM");
    } catch {}
    autoProc = null;
  }
  autoBuf = Buffer.alloc(0);
  autoSpeechStreak = 0;
  autoSilenceStreak = 0;
  autoHadSpeech = false;
  if (autoRecordingSince != null) {
    autoRecordingSince = null;
    recording = false;
    forceKillSox();
  }
}

function startAuto(kv, complete, client, toast, systemPrompt, autoSubmit) {
  if (autoProc) return;
  autoEnabled = true;
  const proc = spawn("parec", ["--format=s16le", "--rate=16000", "--channels=1"], {
    stdio: ["ignore", "pipe", "ignore"],
  });
  autoProc = proc;
  const BLOCK = 3200; // 1600 samples * 2 bytes = 100ms

  proc.stdout.on("data", (chunk) => {
    autoBuf = autoBuf.length ? Buffer.concat([autoBuf, chunk]) : chunk;
    while (autoBuf.length >= BLOCK) {
      const block = autoBuf.subarray(0, BLOCK);
      autoBuf = autoBuf.subarray(BLOCK);
      stepAuto(block, kv, complete, client, toast, systemPrompt, autoSubmit);
    }
  });
  ...
  proc.on("error", (err) => {
    autoEnabled = false;
    autoProc = null;
    toast(`Auto voice input failed: ${err.message}`, "error");
  });
  proc.on("exit", () => {
    if (autoProc) {
      autoEnabled = false;
      autoProc = null;
      toast("Auto voice input stopped", "warning");
    }
  });
}

function stepAuto(block, kv, complete, client, toast, systemPrompt, autoSubmit) {
  if (!autoEnabled) return;
  if (ttsPlaying) {
    // The assistant's own voice is on the speakers; don't transcribe it.
    autoSpeechStreak = 0;
    autoSilenceStreak = 0;
    if (autoRecordingSince != null) {
      autoRecordingSince = null;
  ...
  let s2 = 0;
  for (let i = 0; i < block.length; i += 2) s2 += block.readInt16LE(i) ** 2;
  const rms = Math.sqrt(s2 / (block.length / 2));
  const dbfs = rms > 1 ? 20 * Math.log10(rms / 32768) : -120;
  const speaking = dbfs > AUTO_THRESHOLD_DB;

  const toneLevel = goertzelToneLevel(block);
  const toneActive = toneLevel >= TONE_MIN_BIN_LEVEL;
  ...
  if (!recording) {
    ...
    if (speaking) {
      autoSpeechStreak++;
      if (autoSpeechStreak >= AUTO_START_BLOCKS && !soxProc) {
        autoSpeechStreak = 0;
        autoSilenceStreak = 0;
        autoHadSpeech = false;
        autoRecordingSince = Date.now();
        toast("感知中...");
  ...
  // Auto-initiated recording in progress; ignore manual recordings.
  if (autoRecordingSince == null) return;
  ...
  const activeFor = Date.now() - autoRecordingSince;
  const endBySilence = autoHadSpeech && autoSilenceStreak >= AUTO_END_BLOCKS && activeFor >= 300;
  const endByCap = activeFor >= AUTO_MAX_MS;

  if (endBySilence || endByCap) {
    autoRecordingSince = null;
    autoSilenceStreak = 0;
    autoHadSpeech = false;
    doTranscribePipeline(kv, complete, client, toast, systemPrompt, autoSubmit);
  }
}

// ---- Public API for TUI plugin ----

export function setTtsPlaying(v) {
  ttsPlaying = v;
}
...
export function registerSTT(api, kv, complete, prompts, opts) {
  ...
  if (opts?.autoMode === "on") {
    setTimeout(() => startAuto(kv, complete, client, toast, systemPrompt, autoSubmit), 500);
  }

  return [
    {
      title: autoEnabled
        ? "STT: disable auto voice input (hands-free)"
        : "STT: enable auto voice input (hands-free)",
      value: "stt.auto",
      description:
        "Toggle always-on listening: just speak, transcribe (and submit) automatically on silence - no keybinds",
      keybind: "<leader>a",
      slash: { name: "stt-auto" },
      onSelect() {
        if (autoEnabled) {
          stopAuto();
          toast("Auto voice input OFF");
        } else {
          startAuto(kv, complete, client, toast, systemPrompt, autoSubmit);
          toast("自動感知 オン - 話すだけでなく感知します");
        }
      },
    },
    {
      ...
      onSelect() {
        ...
        if (recording) {
          ...
          startRecording(kv, toast);
          if (recording) toast("感知中... もう一度押すと文字起こし");
        }
      },
    },
    {
